...
parser.add_argument(
    "--date", "-d", help="file date YYYY-MM-DD, eg: 2021-02-19, match with date in data"
)
...
""".format(
    table_id=table_id, partition_date=partition_date, fund=code
)
query_job = client.query(dml_statement)  # API request
query_job.result()  # Waits for statement to finish
print("Removed data on {} and fund {} in {}".format(partition_date, code, table_id))

###########
# Add the data to corresponding partition
###########
file_path = "../data/{partition_date}/{file_name}".format(
    partition_date=partition_date, file_name=file_name
)
...
print("Loaded data to {} on {} for {}".format(table_id, partition_date, code))
